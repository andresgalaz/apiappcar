...
module.exports = function(req, res) {
    ...
    if (!req.body.password && !req.body.google && !req.body.facebook) {
        return res.status(400).json({ success: false, code: 3416, message: 'Falta password.' });
    }
    db.scoreDB.knex("tUsuario")
        .select("pUsuario", "cEmail", "cPassword", "cNombre", "nDni", "dNacimiento", "cSexo", "bConfirmado",
            db.scoreDB.knex.raw("fnUsuarioPerfil(cEmail,?) as bPermiso", [PERFIL]))
        .where("cEmail", req.body.email)
        .then(function(arrUsr) {
            try {
